...
const STATIC_ICONS: { [key: string]: React.ReactNode } = {
  ...
  "paud-terintegrasi": (
    <MessageSquareText className="h-10 w-10 text-brand-teal" />
  ),
...
export default async function ServiceDetailPage({ params }: PageProps) {
  ...
  const icon = STATIC_ICONS[service.id] || (
    <Activity className="h-10 w-10 text-brand-teal" />
  );
  const ageRange =
    service.ageRange || STATIC_AGE_RANGES[service.id] || "Semua Usia";
  ...
  if (Array.isArray(service.advantages)) {
    advantages = service.advantages as string[];
  } else if (typeof service.advantages === "string") {
    try {
      advantages = JSON.parse(service.advantages);
    } catch (e) {
      advantages = [];
    }
  }

  // Fallback to static advantages if database record is empty
  if (advantages.length === 0) {
    advantages = [
      "Dipandu oleh staf medis & terapis berlisensi resmi.",
      "Laporan rekam tumbuh kembang anak berkala.",
      "Fasilitas integrasi sensori standar internasional.",
    ];
  }

  return (
    <>
      <main className="flex-grow pt-12 pb-20 bg-[#F8FAFC]">
        <div className="container mx-auto px-6 max-w-5xl">
          {/* Back Button & Breadcrumbs row */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <Link
              href="/"
              className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-brand-teal transition-colors group focus:outline-none"
            >
              <ArrowLeft className="h-4 w-4 transition-transform group-hover:-translate-x-1" />
              <span>Kembali ke Beranda</span>
            </Link>

            <nav className="flex items-center gap-2 text-slate-400 text-xs">
              <Link
                href="/"
                className="hover:text-brand-teal transition-colors flex items-center gap-1 font-light"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
            ...
            <div className="md:col-span-5 space-y-6">
              <div className="bg-white border border-slate-200/60 shadow-[0_10px_40px_rgba(13,92,102,0.02)] rounded-3xl p-6 flex flex-col items-center text-center">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-xs font-semibold border ${badgeClass} mb-4`}
                >
                  {badgeText}
                </span>
            ...
            <div className="md:col-span-7 bg-white border border-slate-200/60 shadow-[0_10px_40px_rgba(13,92,102,0.03)] rounded-3xl p-8 space-y-6">
              <div>
                ...
                <p className="text-xs text-slate-400 font-light">
                  Gentala Child Development Center
                </p>
              </div>
              ...
              {customFields.length > 0 && (
                <div className="space-y-3 pt-4 border-t border-slate-100">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">
                    Formulir Kustom Tambahan
                  </h3>
                  <p className="text-[10px] text-slate-455 font-light leading-normal mb-2">
                    Untuk menunjang asesmen awal, pendaftaran program ini
                    memerlukan informasi tambahan berupa:
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {customFields.map((field, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-2 p-3 bg-slate-50/50 border border-slate-100 rounded-xl"
                      >
